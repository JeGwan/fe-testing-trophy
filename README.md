...
```
                E2E                        (예정 — Step 5+)
        ─────────────────────
        Component / Integration            ← Step 3  (jsdom + RTL, 3 테스트)
    ─────────────────────────────
...
### 3. Component / Integration — 가상 DOM 렌더 (Step 3 도입)

트로피에서 **가장 두껍게 두라고 하는 층**. 컴포넌트가 React 트리 + DOM + 접근성 트리와 함께 엮여 동작하는 모습을 검증하므로, 한 번에 잡히는 결함의 범위가 unit보다 훨씬 넓다.

- **도구**: Vitest + **jsdom** + `@testing-library/react` (RTL) + `@testing-library/jest-dom`
...
| 4 | (예정) 사용자 인터랙션 | Component / Integration 확장 | `@testing-library/user-event` | TBD |
