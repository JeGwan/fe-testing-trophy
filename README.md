...
```
                E2E                        ← Step 6 (Playwright + Chromium, 2 테스트)
        ─────────────────────
        Component / Integration            ← Step 3 + 4 + 5 (jsdom + RTL + user-event, 10 테스트)
    ─────────────────────────────
                Unit                       ← Step 2 + 5 (pure 함수, 15 테스트)
        ─────────────────────
...
### 3. Component / Integration — 가상 DOM 렌더 (Step 3 + 4 + 5 도입)
...
진짜 브라우저(Chromium)에서 진짜 앱(`next start`)을 띄워 사용자처럼 조작. 트로피의 꼭대기, 수가 적고 핵심 흐름만.

- **도구**: Playwright (Next.js 공식 권장)
